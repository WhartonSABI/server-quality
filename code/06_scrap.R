...
# --- Config ---
outcome_type <- "serve_efficiency_outcome"  # serve_efficiency_outcome or win_pct_outcome
tournament_gender_tag <- "wimbledon_males"  # wimbledon_males, wimbledon_females, usopen_males, usopen_females
column_name <- "serve_efficiency"  # serve_efficiency, win_rate
...
plot <- ggplot(df, aes_string(x = column_name)) +
    geom_histogram(color = "white", fill = "steelblue", bins = 30, alpha = 0.8) +
